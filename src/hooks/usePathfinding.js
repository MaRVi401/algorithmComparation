...
import { ROWS, COLS, TOTAL_EPISODES, HYBRID_EPISODES } from "../constants";
...
export const usePathfinding = () => {
    ...
    for (let r = 0; r < ROWS; r++) {
      ...
      for (let c = 0; c < COLS; c++) {
        ...
        row.push({
          ...
          isEnd: r === 10 && c === 35,
          isWall:
            (r === 10 && c === 5) || (r === 10 && c === 35) ? false : isWall,
          isVisited: false,
          isPath: false,
        });
      }
  ...
  const finishPath = async (node, fGrid, startT, vCount, mlPath = null) => {
    ...
    setStats({
      ...
      time: (performance.now() - startT).toFixed(2),
    });
    setIsRunning(false);
  };
  ...
  const runAlgorithm = async (type) => {
    ...
    // PENANGANAN UNTUK Q-LEARNING DAN HYBRID METHOD
    if (type === "Q-Learning" || type === "Hybrid") {
      setProgress(0.1);
      setIsTrainingComplete(false);
      stopTrainingRef.current = false;

      const isHybrid = type === "Hybrid";
      const targetEpisodes = isHybrid ? HYBRID_EPISODES : TOTAL_EPISODES;

      const ALPHA = 0.2;
      const GAMMA = 0.99;
      let epsilon = 1.0;
      const MAX_STEPS = 1000;
      const CHUNK_SIZE = isHybrid ? 500 : 1000;
      const qTable = {};
      ...
      const getDist = (r, c) =>
        Math.abs(r - endNode.r) + Math.abs(c - endNode.c);
      ...
      for (let i = 0; i < targetEpisodes; i++) {
        ...
        while (
          !(curr.r === endNode.r && curr.c === endNode.c) &&
          steps < MAX_STEPS
        ) {
          ...
          if (Math.random() < epsilon) {
            ...
            action = values.every((v) => v === values[0])
          ...
          if (
            ...
            nextC < 0 ||
            nextC >= COLS ||
            workingGrid[nextR][nextC].isWall
          ) {
            const reward = -15;
            qTable[stateKey][action] +=
              ALPHA * (reward - qTable[stateKey][action]);
            break;
          }
          ...
          // REWARD SHAPING HIBRIDA: Injeksi Heuristik A*
          let reward = nextDist < currentDist ? 1 : -1.5;
          if (isHybrid) {
            const heuristicBonus = (currentDist - nextDist) * 2.0;
            reward += heuristicBonus;
          }
          ...
          qTable[stateKey][action] +=
            ALPHA * (reward + GAMMA * maxNextQ - qTable[stateKey][action]);
        ...
        if (i % CHUNK_SIZE === 0) {
          setProgress(Math.round((i / targetEpisodes) * 100));
          setTrainingData({
            episode: i,
            epsilon: epsilon,
            success: goalReachedCount,
          });
      ...
      while (openSet.length > 0) {
        ...
        if (!workingGrid[current.r][current.c].isStart) {
          ...
          const nodeEl = document.getElementById(
            `node-${current.r}-${current.c}`,
          );
          if (nodeEl)
            gsap.to(nodeEl, {
              backgroundColor: type === "A*" ? "#818cf8" : "#22d3ee",
              scale: 0.8,
              borderRadius: "50%",
              duration: 0.1,
            });
      ...
      setErrorMsg({
        ...
        body: `The ${type} algorithm has exhausted all possibilities, but no valid path exists.`,
      });
      setShowError(true);
    }
  };
  ...
  const handleVisualizeML = async () => {
    ...
    while (
      !(temp.r === endNode.r && temp.c === endNode.c) &&
